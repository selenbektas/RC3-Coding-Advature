...
public class Solver2D : MonoBehaviour
{
    ...
    Dictionary<Vector2Int, int> checkedNodesOptionCount;
    ...
    public void Step()
    {
        ...
        Tile2D selectedTile = tileSet[tileIndex];
        ...
        if (unsolved.Contains(behind))
        {
            ...
            if (checkedNodesOptionCount.ContainsKey(behind))
            {
                checkedNodesOptionCount[behind] = optionCount;
            }
            else
            {
                checkedNodesOptionCount.Add(behind, optionCount);
            }
    ...
    float AdvancedEvaluate(Tile2D tile, Vector2Int id)
    {
        ...
        mapGray = Mathf.Lerp(mapGrayValueOffset, 1.0f, mapGray);
